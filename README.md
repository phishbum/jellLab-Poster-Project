...
A three-step Phish concert-memory poster prototype with AI-assisted creative direction and private AI artwork generation.
...
Generated images are stored in a private Blob store under a random UUID plus a 256-bit capability token. The browser stores that unguessable access link only in the device-local draft. Changing the memory, show, or style invalidates the active artwork link but does not delete the persisted generation record.
